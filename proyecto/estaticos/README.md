# Pulso Solidario — Frontend (HTML/CSS)
...
```text
proyecto/
  index.html              Hub que enlaza las 3 pantallas
  ...
  assets/
    css/custom.css        Overrides de marca + componentes sobre Bootstrap 5
...
- **Marca** (`assets/css/custom.css`): variables CSS (`:root`) con la paleta de `DESIGN.md` (teal `#00685f`, rosa `#b90538`, tinta `#0b1c30`), overrides de botones/badges/cards de Bootstrap y componentes propios (sidebar, topbar, tiles de inventario, feed de actividad, tarjetas KPI, mapa, logros, etc.).
- **Tipografía**: Manrope (titulares) + Inter (cuerpo), vía Google Fonts.
- **Responsive**: desktop primero; las rejillas de Bootstrap colapsan a una sola columna en móvil y la barra lateral pasa a horizontal.
...
- Respeto a `prefers-reduced-motion` para la animación del *pulse indicator*.
